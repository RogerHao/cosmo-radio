...
| `cosmoradio-v4-jlceda/manufacturing/` | Gerber / 钻孔 / 坐标 / BOM / PDF / 3D | ⏳ 待从嘉立创EDA导出，见下方清单 |

### 从嘉立创EDA专业版导出制造文件（清单）

> 制造文件无法从 `.epro2` 的 JSON 直接生成，必须在嘉立创EDA专业版 GUI 里导出（参 [官方文档](https://prodocs.lceda.cn/cn/pcb/export-pcb-fabrication-file-gerber/)）。
> 用 v3.2.135 打开 `cosmoradio-v4-r1.0.epro2` 后，逐项导出到 `manufacturing/`：
...
| 5 | **PCB PDF** | `PCB编辑器 → 导出 → PDF/图片` | 含丝印/阻焊层，便于核对 |
...
| J2 | NFC | XH2.54 8P | RC522 mini | CS, SCK, MOSI, MISO, IRQ, GND, VCC, RST |
...
│   └── manufacturing/                 #   ⏳ 待导出 Gerber/BOM/坐标/PDF/3D
